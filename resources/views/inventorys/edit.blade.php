...
<div class="mx-4 my-8">
    ...
    <form action="{{ route('inventorys.update', $inventoryItem->idbarang) }}" method="POST" class="no-select">
        ...
        <div class="mb-4">
            <label for="idPengguna" class="block text-gray-700">Lecturer</label>
            <select name="idPengguna" id="idPengguna" class="border border-gray-300 rounded w-full px-3 py-2">
                @foreach ($penggunas as $pengguna)
                <option value="{{ $pengguna->idPengguna }}" {{ $inventoryItem->idPengguna === $pengguna->idPengguna ? 'selected' : '' }}>{{ $pengguna->namaPengguna }}</option>
                @endforeach
            </select>
        </div>
        <button href="{{ route('inventorys.index') }}" class="bg-slate-700 text-white px-4 py-2 rounded-md">Cancel</button>
