...
@section('container')
    <div class="mx-4 my-8">
        ...
        <a href="{{ route('penggunas.create_students') }}" class="bg-slate-700 hover:bg-opacity-90 text-white px-4 py-2 rounded-md mb-4 inline-block">Add Students</a>
                ...
                @if($pengguna->role == 'Mahasiswa')
                    <tr class="{{ $loop->odd ? 'bg-gray-200' : 'bg-gray-300' }}">
                        ...
                        <td class="py-2 px-4 border-b text-center">
                            <a href="{{ route('penggunas.edit_students', $pengguna->idPengguna) }}" class="text-yellow-500">Edit</a>
                            <a class="mx-1">|</a>
                            <form action="{{ route('penggunas.destroy_students', $pengguna->idPengguna) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
